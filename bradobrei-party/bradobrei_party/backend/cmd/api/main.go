...
func main() {
	...
	r.Use(cors.New(cors.Config{
		// В домашнем/туннельном деплое origin может быть любым (LAN IP, tunnel URL, домен).
		// Токен передаётся в Authorization, не через cookie, поэтому разрешаем все origin.
	...
	auth := v1.Group("/auth")
	{
		...
		auth.POST("/activate-employee", authH.ActivateEmployee)
	}
	...
	api.Use(middleware.AuthRequired(jwtSecret))
	{
		...
		emps := api.Group("/employees")
		{
			...
			emps.PATCH("/me/profile", middleware.RequireRoles(
				models.RoleBasicMaster, models.RoleAdvancedMaster, models.RoleHR,
				models.RoleAccountant, models.RoleNetworkManager, models.RoleAdmin,
			), employeeH.PatchMeProfile)
