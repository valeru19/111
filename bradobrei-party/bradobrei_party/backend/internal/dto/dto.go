...
type HireEmployeeRequest struct {
	...
	Password string          `json:"password" binding:"omitempty,min=6" example:"password"`
	...
	MustSetPassword bool   `json:"-"`
}

// ActivateEmployeeRequest — первый вход сотрудника: задать пароль для учётки, созданной без пароля.
type ActivateEmployeeRequest struct {
	Username string `json:"username" binding:"required,min=3,max=50" example:"master_ivan"`
	Password string `json:"password" binding:"required,min=6" example:"password"`
}

// PatchMyProfileRequest — поля профиля, которые сотрудник может менять сам.
