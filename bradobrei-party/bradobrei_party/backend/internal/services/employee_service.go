...
func (s *EmployeeService) HireEmployee(req dto.HireEmployeeRequest) (*models.EmployeeProfile, error) {
	...
	user := &models.User{
		...
		MustSetPassword: req.MustSetPassword,
...
// PatchMyProfile обновляет ограниченный набор полей текущего сотрудника.
func (s *EmployeeService) PatchMyProfile(userID uint, req dto.PatchMyProfileRequest) (*models.EmployeeProfile, error) {
	...
	if err != nil {
		return nil, errors.New("учётная запись не найдена")
	}
