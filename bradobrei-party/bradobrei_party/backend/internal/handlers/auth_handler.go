...
// @Summary Первый вход сотрудника (задать пароль)
// @Description Для учётной записи, созданной админом без пароля: задаёт пароль и возвращает JWT.
// @Tags auth
// @Accept json
// @Produce json
// @Param request body dto.ActivateEmployeeRequest true "Логин и новый пароль"
// @Success 200 {object} dto.LoginResponse
// @Failure 400 {object} dto.ErrorResponse
// @Router /auth/activate-employee [post]
func (h *AuthHandler) ActivateEmployee(c *gin.Context) {
	...
	resp, err := h.authService.ActivateEmployee(req.Username, req.Password)
	if err != nil {
		c.JSON(http.StatusBadRequest, dto.ErrorResponse{
